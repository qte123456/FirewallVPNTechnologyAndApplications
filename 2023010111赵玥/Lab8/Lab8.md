...
| `wan` 无法回复的原因 |wan 没有到 10.0.0.0/24 的回程路由，无法将应答包发回内网 |
...
答：conntrack 还额外记录了内网原始地址与NAT转换后公网地址的映射关系。这说明 conntrack 是 Linux 状态防火墙与 NAT 共同的底层核心机制，两者共享同一张连接跟踪表，NAT依赖它做地址转换，状态检测依赖它做连接合法性判断。

2. 如果两台内网主机同时访问同一个外网服务器的同一个端口，NAT 如何区分两条连接的回复分别属于哪台内网主机？
答：NAT 会为每一条连接分配 不同的源端口号，并在 conntrack 中记录完整五元组（源 IP、源端口、目的 IP、目的端口、协议）。当回复包返回时，NAT 根据 目的端口 + IP 查表，即可准确对应到内网主机与端口。
...
| TCP 38070 访问是否成功 |成功 |
| ping 是否成功 |失败 |
| ping 失败的原因 |防火墙 FORWARD 链默认 DROP，且只放行了 TCP 38070 的 NEW 连接，未放行 ICMP 协议|
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址在互联网上没有路由，公网路由器不会转发目的为私有地址的数据包，且多内网复用同一私网段会造成冲突。NAT 在出口将内网源地址改为防火墙公网地址，让外网以为是防火墙发起访问，回复包发往防火墙，再由 NAT 根据映射转发给内网主机。


2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：出口 IP 固定时用 SNAT，性能更高；出口 IP 动态变化（如拨号、4G/5G）时用 MASQUERADE，它会自动获取接口当前 IP，无需手动修改规则。


3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：FTP、SIP、P2P 下载等。因为这些协议会在应用层消息里携带内网 IP / 端口，NAT 只修改 IP 包头，不改应用层内容，导致外网收到无效内网地址而无法通信。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：因为两者共用同一套连接跟踪表，集成后只需维护一份连接状态，效率更高、配置更简单，同时实现安全访问与多设备共享上网，符合家用路由需求。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：sudo ip netns exec fw iptables -A FORWARD -p icmp --icmp-type echo-request -m conntrack --ctstate NEW -j ACCEPT
