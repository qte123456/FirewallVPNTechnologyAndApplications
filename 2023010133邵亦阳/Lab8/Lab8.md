...
| `wan` 无法回复的原因 |1. wan 命名空间没有回程路由：收到 10.0.0.0/24 网段的 ICMP 请求，但路由表中没有 10.0.0.0/24 的路由，不知道怎么把 ICMP 回复包发回给 lan；2. fw 命名空间未开启 IP 转发：fw 作为 lan 和 wan 之间的中转节点，没有开启内核转发，无法转发两个网段之间的数据包；3. fw 网卡未配置 IP：fw 的两个网卡没有配置对应网段的 IP，无法作为网关转发数据包。 |
...
| TCP 38070 访问是否成功 | 是|
| ping 是否成功 |否 |
| ping 失败的原因 |防火墙的 iptables 规则仅允许了 TCP 38070 端口的流量通过，没有放行 ICMP 协议的流量，导致 ICMP echo request（ping 请求）包被防火墙规则拦截，无法正常转发，因此 ping 无法成功。 |
...
1. 为什么私有地址（如 `10.0.0.0/8`）的主机通常无法被互联网直接访问？NAT 是怎么解决这个问题的？
   ...
   > 答：私有地址如 10.0.0.0/8 属于内网保留地址，不会在公网路由器上进行路由，互联网路由器会直接丢弃目标为私有地址的数据包，因此私有地址主机无法被互联网直接访问。NAT 通过在出口网关将内网主机的私有源地址替换成公网可路由的地址，让数据包能够正常在互联网传输，当回复包返回时，NAT 再将目的地址还原为内网私有地址，从而实现内网主机访问互联网的功能。

2. `MASQUERADE` 和 `SNAT --to-source 192.0.2.1` 在本实验中效果相同。实际场景中应如何选择？

   > 答：SNAT --to-source 适合公网 IP 固定不变的场景，能够明确指定转换后的地址，效率更高；MASQUERADE 会自动使用接口当前的 IP 地址，适合动态获取公网 IP 的环境，例如家庭宽带拨号。在实验环境中两者效果相同，但实际部署时，静态公网 IP 用 SNAT，动态 IP 则使用 MASQUERADE。

3. NAT 对外隐藏了内网拓扑，但它给哪些应用层协议带来了麻烦？为什么？

   > 答：NAT 会对内嵌 IP 地址和端口信息的应用层协议造成影响，例如 FTP、SIP、RTSP、TFTP 等，因为这些协议会在数据包的数据部分携带内网地址和端口，而 NAT 只修改 IP 头部的地址，不会自动修改数据部分的地址信息，导致接收方无法正确解析或建立连接，因此需要专门的应用层网关或协议追踪模块才能正常工作。

4. 状态检测和 NAT 都依赖 conntrack，从这个角度看，为什么家用路由器通常把这两个功能集成在一起？

   > 答：因为状态检测和 NAT 都依赖内核的 conntrack 连接追踪模块，两者共享同一份连接状态信息，不需要重复维护连接表，集成在一起可以简化实现、提升效率，同时家用路由器既需要 NAT 实现多设备共享上网，也需要状态防火墙保障内网安全，因此将两者结合是最合理高效的设计。

5. 任务五里 ping 被 FORWARD 链拦截。如果要在保留现有规则的基础上，让 ICMP 也能通过，应该怎么写规则？

   > 答：在防火墙的 FORWARD 链中添加允许 ICMP 协议转发的规则，同时允许 conntrack 追踪的相关状态通过，命令为 iptables -A FORWARD -p icmp -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT，这样既不影响原有安全规则，又能让 ping 正常转发。
