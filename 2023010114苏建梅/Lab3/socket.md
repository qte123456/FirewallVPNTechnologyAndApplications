...
| :--- | :------------------ | :--------------------- |
| `server.py` 已启动，`client.py` 尚未运行 |LISTEN |阶段 1：创建套接字（socket + bind + listen）|
| `client.py` 已连接，数据尚未发送完毕 |ESTABLISHED | 阶段 2：连接（connect / accept 完成）|
| `server.py` 执行 `close()` 之后 | FIN-WAIT-2 和 CLOSE-WAIT|阶段 4：断开（close 触发四次挥手） |
...
| 响应的第一行（状态行） |Connection: close |
| 程序是否自动退出（不需要手动 Ctrl+C）|是 |
| 是你调用 `close()` 触发的断开，还是服务器主动断开的 |服务器主动断开（因为响应头中有 Connection: close） |
...
| :--- | :------- | :------- |
| 程序是否自动退出 |是 | 否|
| `ss` 中连接的 State | TIME_WAIT 或连接已消失| ESTAB|
...
1. 实验中 `server.py` 必须比 `client.py` 先启动。如果顺序反过来，`client.py` 会报什么错误？用今天学到的概念解释：此时套接字存在吗？管道存在吗？
   ...
   > 答：如果 `client.py` 先启动而 `server.py` 未运行，`client.connect()` 会抛出 `ConnectionRefusedError`。此时客户端套接字存在（已被创建），但没有管道（即TCP连接），因为服务器端没有套接字在监听端口，导致三次握手无法完成。

2. `server.py` 打印出了两个不同的描述符编号（`server` 和 `conn`）。为什么 `accept()` 要返回一个新的套接字，而不是直接复用原来的 `server` 套接字？

   > 答：因为监听套接字（`server`）和已连接套接字（`conn`）职责不同。`server` 套接字负责在固定端口上监听新连接，它不能同时处理与特定客户端的数据传输，否则会阻塞其他客户端的连接请求。`accept()` 返回的新套接字专门用于与单个客户端通信，这样 `server` 套接字可以继续监听并接受更多连接，实现并发处理。

3. 描述符和端口号都可以用来标识一个套接字，它们的本质区别是什么？各自解决了什么问题？

   > 答：描述符是操作系统分配给进程的内部整数，用于在进程内部唯一标识一个已打开的文件或套接字，解决的是“进程如何操作内核对象”的问题；端口号是网络层的标识符，用于在主机间定位特定服务或进程，解决的是“数据包如何路由到正确应用”的问题。描述符是进程本地的句柄，不同进程可以有相同数值的描述符；端口号是全局统一的，同一主机上端口号不能重复绑定。简言之，描述符让内核知道数据发给哪个进程的哪个套接字，端口号让网络栈知道数据发给主机的哪个服务。

4. `client.py` 中，你有没有指定过客户端自己的端口号？这个端口号是谁分配的？在 `ss` 的输出中能看到它吗？

   > 答：没有在 client.py 中指定过客户端端口号。这个端口号是操作系统内核自动分配的，具体是从临时端口范围（通常 32768-60999）中随机选择一个。

5. HTTP/1.0 实验中，即使你没有调用 `close()`，`recv()` 循环也会自动退出。解释这是为什么？

   > 答：这是因为 HTTP/1.0 的服务器在发送完响应数据后，**默认会主动关闭连接**。即使你的客户端程序没有调用 `close()`，服务器那一端完成了响应发送，就会关闭其端的 socket。这个关闭操作会通过 TCP 协议栈传递到客户端，表现为接收到一个 **EOF**（文件结束标志）。

此时，客户端的 `recv()` 会返回 **0**（而不是 -1 错误），表示“对端已关闭连接”。你的循环检测到返回值为 0 后就自然退出了，无需显式调用 `close()`。

6. HTTP/1.1 实验中，程序卡在 `recv()` 不退出，`ss` 显示连接仍是 `ESTABLISHED`。这说明 HTTP/1.1 和 HTTP/1.0 在连接管理上有什么根本区别？

   > 答：这说明 HTTP/1.1 默认使用**持久连接**（Keep-Alive），而 HTTP/1.0 默认是**短连接**。程序卡在 `recv()` 是因为服务器在发送完响应后**没有主动关闭连接**，客户端仍在等待更多数据。在 HTTP/1.1 中这是正常行为（连接可复用），客户端需要根据 `Content-Length` 或分块编码判断响应结束，而不是依赖连接关闭。而 HTTP/1.0 默认发送完数据后服务器会关闭连接，导致 `recv()` 返回 0 从而退出。
