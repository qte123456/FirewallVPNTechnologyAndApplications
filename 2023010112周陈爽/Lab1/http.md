...
2. 状态码 `404` 代表什么含义？状态码 `500` 和 `503` 有什么区别？
   ...
   > 答：404 Not Found：代表服务器无法找到请求的资源，通常是 URL 输入错误、资源已删除或路径不存在。
500 Internal Server Error：服务器在处理请求时发生了未知错误，属于服务器端代码 / 配置异常（如代码 Bug、数据库连接失败）。
503 Service Unavailable：服务器暂时无法处理请求，通常是过载、维护中或资源临时不足，属于临时状态，一般会附带重试提示。
区别：500 是 “服务器内部出错”，503 是 “服务器暂时不可用”，前者多为程序问题，后者多为负载或运维问题。


3. GET 与 POST 方法的主要区别是什么？各适用于什么场景？

   > 答：区别：
GET 数据拼在 URL 里，长度受限、可见、可缓存；POST 数据放在请求体，长度无限制、相对更安全、一般不缓存。
GET 语义是获取资源（幂等），POST 语义是提交 / 修改数据（非幂等）。
适用场景：
GET：查询、浏览类操作（如搜索、查看页面）。
POST：提交表单、上传文件、修改数据等操作（如登录、发布内容）。

4. HTTP 与 HTTPS 有什么区别？HTTPS 使用了什么机制来保护数据？

   > 答：区别：
HTTP 明文传输，端口 80，无身份验证；HTTPS 加密传输，端口 443，需 CA 证书验证身份。
HTTPS 保护机制：
用对称加密传输数据，非对称加密交换密钥；
通过 CA 证书验证服务器身份；
用摘要算法校验数据完整性，防止篡改。
...
5. 既然 HTTPS 已经加密，为什么浏览器开发者工具仍然能看到请求和响应的明文内容？
   ...
   > 答：HTTPS 加密是 ** 端到端（浏览器 ↔ 服务器）** 的加密：
加密发生在网络传输层，数据在离开浏览器后、到达服务器前是加密状态。
浏览器开发者工具是浏览器自身的调试工具，它工作在应用层，能看到加密前的请求明文和解密后的响应明文，这是浏览器处理数据的原始内容。
简单来说：开发者工具看到的是 “浏览器内部处理的数据”，而不是 “网络上传输的加密数据”，所以即使 HTTPS 加密，开发者工具仍能查看明文。
