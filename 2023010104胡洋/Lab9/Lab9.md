...
| 测试项 | 结果（成功/失败） | 现象描述 |
| :----- | :-------------- | :------- |
| `office -> dmz:8080` |成功 |执行curl后返回 HTTP目录列表，显示dmz命名空间内的Python HTTP服务器正常响应，连接建立成功。 |
| `guest -> dmz:8080` |失败 |执行 curl 后提示Failed to connect to 10.0.3.2 port 8080: Could not connect to server，连接被拒绝，无法访问DMZ区的8080服务。 |
| `guest -> office` |失败 |执行curl后提示Failed to connect to 10.0.1.2 port 8080: Could not connect to server，连接被拒绝，无法访问Office区的服务。 |
...
> 答：不会命中。因为iptables规则是按顺序匹配的，数据包命中REJECT规则后会直接被拒绝并丢弃，不会继续匹配后面的LOG规则。
...
> 答：合理，因为iptables中只有针对guest网段的LOG规则，office 网段的流量命中了ACCEPT 规则，没有对应的LOG规则，所以不会生成日志。要记录所有被放行的流量，需要在ACCEPT规则前，为 office -> dmz 的流量添加一条LOG规则。

2. 从日志里你能读出什么信息？"谁在访问谁的什么服务"能还原出来吗？

> 答：可以还原。日志包含了源 IP、目标 IP、目标端口等信息。例如日志中 SRC=10.0.2.2（guest）访问 DST=10.0.3.2:8080（dmz 的 8080 服务），前缀 GUEST-TO-DMZ 也明确了访问方向。
...
| 项目 | 你的填写 |
| :--- | :------- |
| `GUEST-TO-DMZ` 总日志条数 |3 |
| `GUEST-TO-OFFICE` 总日志条数 |2 |
| 循环 5 次后 `GUEST-TO-DMZ` 增加了多少 |0 |
| 你使用的过滤命令 |sudo journalctl -k --grep "GUEST-TO-DMZ" --no-pager或者sudo journalctl -k --grep "GUEST-TO-OFFICE" --no-pager   | 
...
> 答：通过不同前缀给事件 “打标签”，可以快速分类、过滤不同类型的日志，方便故障排查、安全审计和攻击溯源，也便于后续自动化脚本处理不同场景的日志。

2. 如果防火墙每秒拦截上千个包，不加限制地写 LOG 会有什么后果？

> 答：会导致日志量暴增，占用大量磁盘空间，甚至引发磁盘爆满；同时会增加系统 IO 负载，影响防火墙性能，甚至被攻击者利用来发起日志拒绝服务攻击。
...
| 触发访问次数 | 10 |
| 实际写入日志条数 |5 |
| `--limit-burst 5` 的含义 |日志记录的初始最大突发条数为 5，超过后按 --limit 限制的速率记录 |
...
> 答：两者是相互独立的关系。LOG 仅负责按规则记录日志，不影响数据包的流向；REJECT 独立对每个匹配的包执行拒绝操作，不受 LOG 限速的影响。

2. "没记录就当没发生"和"仍然拦截但不记录"有本质区别吗？为什么安全审计中这个区别很重要？

> 答：有本质区别。前者是完全没有安全控制，后者是有控制但审计信息不全。安全审计中，未记录不代表未发生，遗漏日志会掩盖攻击行为，导致无法追溯攻击来源、分析攻击模式，影响事后溯源和安全改进。
...
1. 为什么 `dmz` 的服务不应该和 `office` 混在同一个网段？用本实验的规则效果来说明。
   ...
   > 答：DMZ 是对外服务区，Office 是内部办公网。混在同一网段时，攻击者拿下 DMZ 服务后可直接横向移动到办公网；本实验中 guest 被禁止访问 office，但 DMZ 若与 office 同网段，规则将无法隔离风险。

2. 为什么 `guest` 一般应该和办公网隔离？

   > 答：Guest 网络通常安全等级低，易被攻陷或存在恶意行为；隔离可防止其攻击、扫描或泄露流量波及办公网，避免内部数据被窃取或破坏。

3. 第一阶段中为什么必须保留 `ESTABLISHED,RELATED` 规则？如果去掉会怎样？

   > 答：该规则允许已建立 / 相关的回应包通过，否则 office 访问 dmz 的请求发出后，服务器的回应包会被默认 DROP 策略拦截，导致连接失败。

4. `LOG` target 和 `ACCEPT`/`DROP`/`REJECT` 最本质的区别是什么？为什么 LOG 必须放在 REJECT 前面？

   > 答：LOG 仅记录日志，不改变数据包流向；而 ACCEPT/DROP/REJECT 会决定数据包是否继续转发。如果 LOG 放在 REJECT 后面，数据包被 REJECT 直接丢弃，不会再匹配 LOG规则，日志无法生成。

5. `office -> dmz` 的成功访问没有日志，被拦截的 `guest` 访问有日志。这是合理的还是疏漏？为什么？

   > 答：合理。本实验中日志规则是为了审计被拒绝的非法访问，而 office -> dmz 是被允许的合法访问，默认不记录日志。如果需要审计所有访问，可额外添加 LOG 规则，但会增加日志量。

6. `--limit` 控制日志频率后，超出限制的包仍然被 REJECT 但不再写入日志。这和"只要不记录就当没发生"有本质区别吗？

   > 答：有本质区别。前者只是日志被限速，拦截行为仍在生效，安全控制未失效；后者是既不记录也不拦截，属于无安全控制，存在严重安全隐患。


7. 如果后面要接入 VPN，你认为 VPN 用户应该先接入哪个区域？结合网络分区和日志审计两个角度回答。

   > 答：应先接入 DMZ 区。
网络分区：VPN 用户属于外部访问，先接入 DMZ 可避免直接暴露内部办公网，即使 VPN 被攻破，攻击者也无法直接进入 Office 网段。
日志审计：可在 DMZ 入口统一记录 VPN 用户的访问日志，便于监控和审计外部访问行为，减少内部网的日志噪音。
